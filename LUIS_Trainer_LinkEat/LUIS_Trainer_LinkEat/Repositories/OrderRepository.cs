...
namespace LUIS_Trainer_LinkEat.Repositories
{
    public class OrderRepository
    {
        ...
        {
            var matchingOrders = await dbContext.Orders.Where(order => order.PlaceId == placeId).ToListAsync();
            if (matchingOrders == null) throw new Exception("Impossible to find that order !");

            var dateMatchingOrders = matchingOrders.Where(order => IsSameDate(order.Date, DateTime.Today)).ToList();

            return dateMatchingOrders;
        }

        public async Task<List<Order>> GetByMeal(int mealId)
        {
            var matchingOrders = await dbContext.Orders.Where(order => order.MealId == mealId).ToListAsync();
            if (matchingOrders == null) throw new Exception("Impossible to find that order !");
